...
export default function Topbar({ onMenuClick }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 shrink-0 items-center gap-3 border-b border-slate-200 bg-white/80 px-4 backdrop-blur-sm sm:px-6">
      <button
        onClick={onMenuClick}
      ...
      <div className="relative hidden flex-1 max-w-sm sm:block">
        ...
        <input
          ...
          className="w-full rounded-lg border border-slate-200 bg-slate-50 py-2 ps-9 pe-3 text-sm text-slate-700 placeholder:text-slate-400 outline-none transition-colors focus:border-accent-400 focus:bg-white focus:ring-2 focus:ring-accent-100"
        />
      </div>
      ...
      <div className="flex flex-1 items-center justify-end gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-2.5 border-s border-slate-200 ps-3">
          ...
          <div className="hidden sm:block">
            <p className="text-sm font-medium text-slate-800 leading-tight">{currentUser.fullName[lang]}</p>
